...
IMAGE_URI="https://picsum.photos/200/300/" # URL of image to fetch. Keep in mind that the Kindle 4 does not support SSL/TLS requests
...
BATTERYSLEEP=10800                      # 1 day sleep time when Battery Level is equal or below the "BATTERYLOW" threshold, see above.
PINGHOST="www.google.com"               # which domain (or IP) to ping to check internet connectivity.
ROUTERIP="192.168.178.1"                # router gateway IP. The Kindle appears to sometimes forget the gateway's IP and we need to set this manually.
...
NAME=kkarousel
SCRIPTDIR="/mnt/us/extensions/kkarousel"
